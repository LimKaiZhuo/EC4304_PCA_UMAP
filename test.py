...
from own_package.poos import poos_analysis, poos_processed_data_analysis, poos_experiment, poos_model_evaluation, poos_shap
from own_package.features_labels import read_excel_dataloader, Fl_master, Fl_ar, Fl_pca
    ...
    if case == 1:
        ...
        for h, h_idx in zip(h_store, h_idx_store):
            poos_analysis(fl_master=fl_master, h=h, h_idx=h_idx, model_mode='xgb',
                          results_dir='./results/poos/poos_DPC_xgba',
                          save_dir=f'./results/poos/poos_DPC_xgba/poos_h{h}.pkl')
    elif case == 3:
        model_name = 'xgba'
        poos_processed_data_analysis(
            save_dir_store=[f'./results/poos/poos_DPC_{model_name}/poos_{model_name}_h1_analysis_results.pkl',
                            f'./results/poos/poos_DPC_{model_name}/poos_{model_name}_h3_analysis_results.pkl',
                            f'./results/poos/poos_DPC_{model_name}/poos_{model_name}_h6_analysis_results.pkl',
                            f'./results/poos/poos_DPC_{model_name}/poos_{model_name}_h12_analysis_results.pkl',
                            f'./results/poos/poos_DPC_{model_name}/poos_{model_name}_h24_analysis_results.pkl',
                            ],
            h_store=['1',
                     '3',
                     '6',
            ...
            results_dir=f'./results/poos/poos_DPC_{model_name}')
    elif case == 4:
        ...
        fl_master = Fl_master(x=output[0], features_names=output[1],
                              ...
                              y=output[4], y_names=output[5],
                              time_stamp=output[6])
        fl_xgb = Fl_pca(val_split=None, x=None, yo=None, y=None,
        ...
        poos_experiment(fl_master=fl_master, fl=fl_xgb, est_dates=est_dates, z_type=1, h=1, h_idx=0,
                        m_max=3, p_max=24, model_mode=model_mode, save_dir=results_dir,
                        )
        ...
        poos_model_evaluation(
            ar_store=[f'./results/poos/poos_{var_name}_ar4/poos_ar4_h{h}_analysis_results.pkl' for h in h_store],
        ...
        h_store = [1, 3, 6, 12, 24]
        var_name = 'IND'
        poos_shap(xgb_store=[f'./results/poos/poos_{var_name}_xgba/poos_h{h}.pkl' for h in h_store])

if __name__ == '__main__':
    selector(case=6, excel_dir='./excel/dataset2/DPCERA3M086SBEA_data_loader.xlsx', var_name='poos_DPC_ar4')
